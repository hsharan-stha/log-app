...
@section('content')
    <a href="{{ route('student.courses.show', $course) }}" class="small text-decoration-none">← {{ $course->subject?->name }}</a>
    <h1 class="h3 mt-2 mb-1">{{ $lesson->title }}</h1>
    <p class="text-muted">{{ $lesson->lesson_date?->toFormattedDateString() }} · {{ $course->teacher?->name }}</p>
    ...
    @if($lesson->whiteboards->isNotEmpty())
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white"><strong>Whiteboard</strong></div>
            <div class="card-body">
                <div class="row g-3">
